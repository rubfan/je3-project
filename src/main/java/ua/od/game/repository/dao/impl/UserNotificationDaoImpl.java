...
public class UserNotificationDaoImpl implements UserNotificationDao {
    ...
    public List<Integer> getUserNotifications(Integer userId) {
        return SqlHelper.prepareStatement("select  a.notification_id from (select a.notification_id " +
                "from Trigger_Notification a, User_Resource b " +
                "where a.resource_id = b.resource_id and b.user_id = ? and a.resource_number <= b.number " +
                "union " +
                "select a.notification_id from Trigger_Notification a, User_Upgrade c " +
                "where a.upgrade_id = c.upgrade_id and c.user_id = ? and a.upgrade_number <= c.number " +
                "union " +
                "select a.notification_id from Trigger_Notification a, User_Building d " +
                "where a.building_id = d.building_id and c.user_id = ? and a.building_number <= d.number) as a, " +
