...
public class UserUpgradeDaoImpl implements UserUpgradeDao {
    ...
    public boolean prepareDefaultUserUpgrades(Integer userId) {
        ...
        complete = SqlHelper.prepareStatement("INSERT INTO User_Upgrade(user_id,upgrade_id,number) " +
                "SELECT ? as user_id, id, default_number FROM  Upgrade WHERE default_number <> 0", pstmt ->{
                pstmt.setInt(1,userId);
                pstmt.executeUpdate();
                return true;
        });
